...
doss='TEST_{}'.format(bit_max)
folder='C:\\Users\\MIDAS\\Desktop\\code_UH_long\\GENE_MOD\\ITER89\\'+doss
...
with Session(bitfile,"PXI1Slot2") as session:
    ...
    try:
        print("\nAmplitude : {}".format(int(bit_max)))
        sequence = [pga.Pulse(int(PL*1e6), 0, int(bit_max), int(freq))]
        USshot(sequence)
...
x_abs = [i * 1/ fe*1000 for i in range(len(Data[:end]))]
pl.plot(x_abs[:],Data[:end])
pl.title("Pulse shot amp {} bits".format(bit_max))
pl.ylabel("amplitude")
pl.xlabel("time (ms)")
pl.tight_layout()
pl.savefig(folder_plot+"\\plot_{}.png".format(bit_max))
