...
#VITRO
tra = 'F:\\data_vitro\\EXPE_aout_2023\\ITER_22\\'
traj= 'F:\\data_vitro\\EXPE_aout_2023\\ITER_22\\fin_inertie\\'
path(traj)



legend = ["mouse_564_65"]
nom = ["PULSE_27_75\\", "PULSE_80_75", "PULSE_240_75\\"]
legend = ["pulse 27", "pulse 80", "pulse 240"]
for i in range(3):
    tra0 = tra + nom[i]
    trajet = traj + legend[i]+ "\\"
    path(trajet)
    ...
    print("\nLoading pulses......")
    data_raw = np.load(tra0+'\\data_o.npy') #Bubbles #Control
    print(data_raw.shape)
    data = data_raw
    #data = (np.transpose(data_raw[:,:]))
    #sys.exit()
    print("Loading done!")
    test_exp=experiment(25000000,1500000,start=0,end=-1, size_decal = 1024)#1271
    print("\nAdding pulses exp")
    test_exp.add_pulses(data[-10:, :262016//4], spacer =100e3)
    print("Adding done!")
    x_press = [indi for indi in range(test_exp.pulses[0].n_window)]

    print("\nPLot des différents indices")
    print(legend[i])
    test_exp.plot_indice_RAMP("treatment",legend[i],trajet,x_press)
    std = 1
    print("STD = ", std)
    test_exp.plot_indice_RAMP_std(legend[i],trajet,still_wind = 40, std_tresh = std, true_harm = False, plot_true = True)
sys.exit()
