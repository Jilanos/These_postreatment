...
name = ["Mircen_juin", "Mircen_fev", "NS_1", "NS_2", "NS_3", "NS_4", "NS_5"]
seuil = [1.5, 1.5, 2.5, 2.5, 2.5, 2.5, 1.8]


all_event_log = []
all_event = []
start_i = [0] + [ 10615 for i in range(6)]
end_i = [312500] + [  for i in range(6)]
fe_i =  [2*15.625e6] + [ 25000000 for i in range(6)]

for ind, tra in enumerate([tra00, tra0, tra1, tra2, tra3, tra4, tra5]):
    ...
    for j in range(len(amp)):
        ...
        inertial_bbb_norm.append(inertial_bbb_raw[-1]/inertial_dose[amp[j]])
        if inertial_bbb_norm[-1]>seuil[ind] :
...
for i, allev in [all_event_log, all_event]:
    plt.clf()
    color = ['c', 'blue', 'forestgreen', 'lime', 'purple', 'magenta', 'coral']
    style = ['8', 'o', 'D', 's', 'p', 'x', '^']
    ...
    plt.tight_layout()
    plt.yscale('linear')
    plt.savefig("C:\\Users\\PM263553\\Desktop\\muscles_monkey\\" + nom[i] +".png",bbox_inches='tight')    
