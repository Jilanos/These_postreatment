...
doss=["bubbles_0_100","bubbles_25_100","bubbles_50_100","bubbles_75_100","bubbles_99_100"]     
# =============================================================================
...
for i in range(1,nexp):
    ...
    print("adding pulses exp")
    test_exp.add_pulses(data[:30*(bitmax-1)], spacer =100e3)
    test_exp.plot_indice_component(traj+doss[i]+"\\",nbit,fit,30)
    del test_exp
    
    # print("adding pulses multi exp")
    # test_m_exp.add_pulses(data[:30*(bitmax-1)], i, spacer =100e3)
    # print('done')
    del data
    gc.collect(generation=2)
sys.exit()
...
dossier = traj+nom+"\\"
legend=["no Mbs","Mbs=0.25mL","Mbs=0.50mL","Mbs=0.75mL","Mbs=1.00mL"]
test_m_exp.plot_indice_together_grp(nom,dossier,nbit,30,legend,fit = list(fit))
